...
<script>
    ...
    function checkNIK(name, value){
        $.ajax({
            type: "POST",
            url : "{{route('check-nik-service-requisition')}}",
            data : {
                'nik'      : value, 
                '_token'   : '{{csrf_token()}}'
            },
            success: function(msg){
                if(msg == true){
                    alert('NIK Pemohon Sudah Mendapat Bantuan Tahun Lalu / Ini');
                }
            }
        });
    }
    ...
    function checkName(name, value){
        $.ajax({
            type: "POST",
            url : "{{route('check-name-service-requisition')}}",
            data : {
                'name'      : value, 
                '_token'    : '{{csrf_token()}}'
            },
            success: function(msg){
                if(msg == true){
                    alert('Nama Pemohon Sudah Mendapat Bantuan Tahun Lalu / Ini');
                }
            }
        });
    }
    ...
    function checkAddress(name, value){
        $.ajax({
            type: "POST",
            url : "{{route('check-address-service-requisition')}}",
            data : {
                'address'   : value, 
                '_token'    : '{{csrf_token()}}'
            },
            success: function(msg){
                if(msg == true){
                    alert('Alamat Pemohon Sudah Mendapat Bantuan Tahun Lalu / Ini');
                }
            }
        });
    }
                ...
                <div class="row form-group">
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input class="form-control input-bb" type="number" name="service_requisition_nik" id="service_requisition_nik" value="{{$servicerequisition['service_requisition_nik']}}" onChange="checkNIK(this.name, this.value);" autocomplete="off"/>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <div class="form-group">
                    ...
                    <div class="col-md-6">	
                        <div class="form-group">	
                            ...
                            {!! Form::select('kecamatan_id',  $corekecamatan, $servicerequisition['kecamatan_id'], ['class' => 'selection-search-clear select-form', 'id' => 'kecamatan_id', 'name' => 'kecamatan_id']) !!}
                        </div>
                    </div>
                    <div class="col-md-6">	
                        <div class="form-group">	
                            <a class="text-dark">Kelurahan</a>
                            {!! Form::select('kelurahan_id',  $corekelurahan, $servicerequisition['kelurahan_id'], ['class' => 'selection-search-clear select-form', 'id' => 'kelurahan_id', 'name' => 'kelurahan_id']) !!}
                        </div>
                    </div>
                    <div class="col-md-12">
                        <div class="form-group">
                            <a class="text-dark">Alamat Pemohon</a>
                            <textarea class="form-control input-bb" type="text" name="service_requisition_address" id="service_requisition_address" onChange="checkAddress(this.name, this.value);" autocomplete="off">{{$servicerequisition['service_requisition_address']}}</textarea>
                        </div>
                    </div>
                </div>
